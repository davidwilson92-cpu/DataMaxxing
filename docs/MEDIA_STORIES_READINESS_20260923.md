...
## Validation
- Final local suite: 180 passed, 17 existing dependency/test-client deprecation warnings. Tests use disposable data and block unmocked external HTTP.
...
- Additive migration runs twice against an old synthetic schema and preserves records. Existing local SQLite/upload/hash/credential restore test passes. CI PostgreSQL/hosting validation is tracked separately.
