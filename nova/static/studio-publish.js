...
async function requestPublishConfirmation(mode,scheduledLocal=null,targets=null){
  ...
  try{
    ...
    if(blocked.length){
      ...
      document.getElementById('reviewPanel').innerHTML=`<section class="chat-confirmation"><h3>Choose what to review now</h3><p>${blocked.map(platformName).join(', ')} need a connection or permission check. Their versions remain saved here.</p><a href="/account#socials">Manage connections</a>${ready.length?`<button class="button primary" onclick="requestPublishConfirmation('${mode}',null,${esc(JSON.stringify(ready))})">Review ${ready.map(platformName).join(', ')} only</button>`:''}</section>`;openReview();return;
    }
    ...
    document.getElementById('reviewPanel').insertAdjacentHTML('beforeend',`<section class="chat-confirmation"><h3>Review destinations and settings</h3>${controls}<button class="button primary" onclick="buildPublicationReview()">Review final post</button></section>`);openReview();
...
async function confirmChatPublish(button){
  ...
  try{
    ...
    currentDraftStatus=result.draft_status;invalidateReview();refreshDraft();showPublicationResults(result);loadHistory();
  }catch(error){addAssistantMessage(error.message);}
