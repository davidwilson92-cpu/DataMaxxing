...
function draftWorkspace(){const platforms=Object.keys(variants),missing=["x","instagram","facebook","tiktok"].filter(p=>!variants[p]),locked=!editableDraft();return `<section class="inline-draft" data-current-draft><div class="draft-tabs" role="tablist" aria-label="Generated platform versions">${platforms.map(p=>`<button role="tab" aria-selected="${p===currentPlatform}" class="draft-tab ${p===currentPlatform?"active":""}" onclick="switchPlatform('${p}')">${platformIcon(p)} ${platformName(p)} <span>${locked?esc(publicationStates[p]?publicationStatus(publicationStates[p]):"Not submitted"):"Draft"}</span></button>`).join("")}<span id="autosaveStatus" class="autosave-status">${locked?currentDraftStatus:"Saved"}</span></div><div id="variantEditor">${editorMarkup()}</div>${locked?"":`<div class="draft-actions"><span class="action-label">Refine in chat</span><div class="refine-chips"><button onclick="promptComposer('Make the ${platformName(currentPlatform)} version shorter')">Shorter</button><button onclick="promptComposer('Make the ${platformName(currentPlatform)} version more serious')">More serious</button><button onclick="promptComposer('Make the ${platformName(currentPlatform)} version more playful')">More playful</button><button onclick="promptComposer('Make the ${platformName(currentPlatform)} version clearer without adding facts')">Clarify, keep the facts</button></div>${missing.length?`<div class="add-platforms"><span>Add another version</span>${missing.map(p=>`<button onclick="promptComposer('Add a ${platformName(p)} version of this post')">${platformIcon(p)} ${platformName(p)}</button>`).join("")}</div>`:""}</div>`}<div class="publish-row"><span>${platforms.length} version${platforms.length===1?"":"s"} ${locked?"· see results below":"to review"}</span><div><a class="text-button" href="/drafts">All drafts</a>${locked?"":`<button class="button ghost" onclick="requestPublishConfirmation('schedule')">Schedule</button><button class="button primary" onclick="requestPublishConfirmation('publish')">Review & publish</button>`}</div></div></section>`;}
...
function renderAttachments(){
  ...
  document.getElementById("mediaStatus").textContent=uploadedMedia.length?(uploadedMediaKind==="video"?"Video attached · Instagram and TikTok only":`${uploadedMedia.length} image${uploadedMedia.length===1?"":"s"} attached`):"Media and source links are not inspected. Describe their relevant details for Zova.";
